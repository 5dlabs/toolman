...
use crate::session::{SessionContext, SessionInitRequest, SessionInitResponse, SpawnedServerInfo, ServerStatus, GlobalServerInfo, AvailableToolInfo, SessionCapabilities, SessionFeatures, ProcessInfo};
...
impl SessionStore {
    ...
    pub async fn create_session(&self, request: SessionInitRequest) -> Result<SessionInitResponse, String> {
        ...
        for server_config in &local_server_configs {
            match self.spawn_local_server_with_process(server_config, &session.working_directory, &session.session_id).await {
                Ok((spawned_info, process)) => {
                    local_servers.insert(server_config.name.clone(), spawned_info.clone());
                    if let Some(proc) = process {
                        spawned_processes.insert(server_config.name.clone(), Arc::new(Mutex::new(proc)));
                    }
                ...
                Err(e) => {
                    ...
                    eprintln!("⚠️ Failed to spawn local server '{}': {}", server_config.name, e);
        ...
        // Store the session
        {
            let mut sessions = self.sessions.write().map_err(|e| format!("Failed to acquire write lock: {e}"))?;
    ...
    pub fn get_session(&self, session_id: &str) -> Result<Option<SessionContext>, String> {
        let sessions = self.sessions.read().map_err(|e| format!("Failed to acquire read lock: {e}"))?;
            ...
            // Remove and cleanup local processes
            {
                ...
                if let Some(session_processes) = processes.remove(session_id) {
                    for (server_name, process_arc) in session_processes {
                        ...
                        println!("🔚 Killing local server process: {} (PID: {})", server_name, 
    ...
    async fn spawn_local_server_with_process(&self, server_config: &crate::session::LocalServerConfig, working_dir: &std::path::Path, session_id: &str) -> Result<(SpawnedServerInfo, Option<LocalServerProcess>), String> {
        println!("🚀 Spawning local server '{}' in {}", server_config.name, working_dir.display());
        ...
        let mut child = cmd.spawn().map_err(|e| {
            format!("Failed to spawn local server '{}': {}", server_config.name, e)
        })?;
        
        let pid = child.id().unwrap_or(0);
        let started_at = SystemTime::now();
        
        println!("✅ Local server '{}' spawned with PID: {}", server_config.name, pid);
        ...
        let (discovered_tools, handshake_success) = self.perform_mcp_handshake_mut(&mut local_process).await;
    ...
    async fn cleanup_local_server(&self, server_name: &str, server_info: &SpawnedServerInfo) -> Result<(), String> {
    ...
    async fn perform_mcp_handshake_mut(&self, process: &mut LocalServerProcess) -> (Vec<String>, bool) {
        println!("🤝 Starting MCP handshake with local server: {}", process.name);
        
        // Implement actual JSON-RPC communication
        match self.execute_mcp_handshake(process).await {
            Ok(tools) => {
                println!("✅ MCP handshake successful with {}: {} tools discovered", process.name, tools.len());
                (tools, true)
            }
            Err(e) => {
                ...
                match self.simulate_mcp_handshake(&process.name).await {
                    Ok(tools) => {
                        println!("🔄 Fallback to simulation successful: {} tools discovered", tools.len());
                        (tools, true)
                    }
                    Err(_) => (vec![], false)
                }
    ...
    async fn execute_mcp_handshake(&self, process: &mut LocalServerProcess) -> Result<Vec<String>, String> {
        ...
        if response.get("error").is_some() {
            return Err(format!("Initialize request failed: {}", response.get("error").unwrap()));
        }
        ...
        println!("📤 Sending initialized notification: {}", notification_line.trim());
        ...
        println!("📤 Sending tools/list request: {}", tools_request_line.trim());
        
        stdin.write_all(tools_request_line.as_bytes()).await
        ...
        stdout.read_line(&mut tools_response_line).await
            .map_err(|e| format!("Failed to read tools/list response: {}", e))?;
        
        println!("📥 Received tools/list response: {}", tools_response_line.trim());
        ...
        if tools_response.get("error").is_some() {
            return Err(format!("Tools/list request failed: {}", tools_response.get("error").unwrap()));
        }
    ...
    fn get_global_servers_info(&self) -> HashMap<String, GlobalServerInfo> {
        ...
        global_servers.insert("web-search".to_string(), GlobalServerInfo {
            tools: vec!["web_search".to_string(), "web_summarize".to_string()],
            status: "running".to_string(),
        });
        
        global_servers.insert("memory".to_string(), GlobalServerInfo {
            tools: vec!["memory_store".to_string(), "memory_retrieve".to_string(), "memory_search".to_string()],
            status: "running".to_string(),
        });
        
        global_servers.insert("database".to_string(), GlobalServerInfo {
            tools: vec!["db_query".to_string(), "db_insert".to_string(), "db_update".to_string()],
            status: "running".to_string(),
        });
    ...
    fn build_available_tools(&self, session: &SessionContext, global_servers: &HashMap<String, GlobalServerInfo>) -> Vec<AvailableToolInfo> {
    ...
    pub async fn discover_local_server_tools(&self, process: &LocalServerProcess) -> Result<Vec<String>, String> {
        self.simulate_mcp_handshake(&process.name).await
    }
    
    /// Get available tools for a specific session
    pub async fn get_available_tools_for_session(&self, session_id: &str) -> Result<Vec<AvailableToolInfo>, String> {
        let session = {
            let sessions = self.sessions.read().map_err(|e| format!("Failed to acquire read lock: {e}"))?;
    ...
    pub async fn execute_tool_on_local_server(&self, session_id: &str, server_name: &str, tool_name: &str, arguments: Value) -> Result<Value, String> {
        println!("🔧 Executing tool '{}' on local server '{}' for session '{}'", tool_name, server_name, session_id);
        ...
        match process_arc {
            Some(process_arc) => {
                let mut process = process_arc.lock().await;
                self.execute_tool_on_process(&mut process, tool_name, arguments).await
            }
            None => {
                Err(format!("No local server process found for session '{}' server '{}'", session_id, server_name))
            }
        }
    }
    
    /// Execute a tool on a specific local server process
    async fn execute_tool_on_process(&self, process: &mut LocalServerProcess, tool_name: &str, arguments: Value) -> Result<Value, String> {
        let stdin = process.stdin.as_mut().ok_or("No stdin available")?;
        let stdout = process.stdout.as_mut().ok_or("No stdout available")?;
...
mod tests {
    ...
    async fn test_tool_aggregation() {
        ...
        let _has_local = response.available_tools.iter()
            .any(|t| t.source.starts_with("local:"));
        let has_global = response.available_tools.iter()
    ...
    async fn test_stale_session_cleanup() {
        ...
        assert_eq!(store.session_count().unwrap(), 0, "Should have 0 sessions after cleanup");
    }
    ...
    async fn test_session_tools_list() {
        ...
        let available_tools = store.get_available_tools_for_session(&session_id).await.unwrap();
        ...
        let read_file_tool = available_tools.iter().find(|tool| tool.name == "read_file").unwrap();
    ...
    async fn test_tool_execution() {
        ...
        let result = store.execute_tool_on_local_server(&session_id, "filesystem", "create_directory", tool_args).await;
        ...
        match result {
            Ok(result_value) => {
                ...
                assert!(std::path::Path::new(&test_subdir_path).exists(), "Directory should have been created");
                println!("✅ Directory verification passed: {} exists", test_subdir_path);
    ...
    async fn test_comprehensive_protocol_communication() {
        ...
        let request = SessionInitRequest {
            ...
            local_servers: vec![
                LocalServerConfig {
                    ...
                    args: vec!["-y".to_string(), "@modelcontextprotocol/server-filesystem".to_string()],
                    env: HashMap::new(),
                    tools: vec!["write_file".to_string(), "read_file".to_string(), "list_directory".to_string()],
                }
            ],
            requested_tools: vec![
                crate::session::ToolRequest {
        ...
        let write_result = store.execute_tool_on_local_server(
            &session_id, 
            "filesystem", 
            "write_file", 
            json!({
        ...
        let read_result = store.execute_tool_on_local_server(
            &session_id,
            "filesystem", 
            "read_file",
            json!({
        ...
        let content_text = read_content.get("content")
        ...
        assert!(content_text.contains("Hello, MCP Protocol Test!"), "File content should match");
        println!("✅ read_file test passed");
        
        // Test 3: list_directory tool
        println!("📂 Test 3: list_directory tool execution");
        let list_result = store.execute_tool_on_local_server(
            &session_id,
            "filesystem",
            "list_directory", 
            json!({
        ...
        let listing_text = list_content.get("content")
            .and_then(|c| c.as_array())
            .and_then(|arr| arr.first())
            .and_then(|item| item.get("text"))
            .and_then(|text| text.as_str())
        ...
        assert!(listing_text.contains("test_file.txt"), "Directory listing should contain our test file");
        println!("✅ list_directory test passed");
        
        // Test 4: Error handling - invalid tool name
        println!("❌ Test 4: Error handling for invalid tool");
        let error_result = store.execute_tool_on_local_server(
            &session_id,
            "filesystem",
            "invalid_tool_name",
            json!({})
        ...
        match error_result {
            Ok(result) => {
                if let Some(is_error) = result.get("isError").and_then(|v| v.as_bool()) {
                    ...
                    println!("✅ Error handling test passed - invalid tool handled as error result");
                } else {
        ...
        let invalid_args_result = store.execute_tool_on_local_server(
            &session_id,
            "filesystem",
            "read_file",
            json!({
    ...
    async fn test_concurrent_tool_execution() {
        ...
        let task1 = {
            ...
            tokio::spawn(async move {
                store_clone.execute_tool_on_local_server(
                    &session_id_clone,
                    "filesystem",
                    "write_file",
                    json!({
    ...
    async fn test_protocol_edge_cases_and_malformed_requests() {
        ...
        let missing_args_result = store.execute_tool_on_local_server(
            &session_id,
            "filesystem",
            "write_file",
            json!({
        ...
        let invalid_type_result = store.execute_tool_on_local_server(
            &session_id,
            "filesystem", 
            "write_file",
            json!({
        ...
        let empty_tool_result = store.execute_tool_on_local_server(
            &session_id,
            "filesystem",
            "", // Empty tool name
            json!({})
        ...
        assert!(empty_tool_result.is_err(), "Empty tool name should return error");
        println!("✅ Empty tool name handled as error");
        
        // Test 4: Nonexistent server
        println!("❌ Test 4: Nonexistent server");
        let nonexistent_server_result = store.execute_tool_on_local_server(
            &session_id,
            "nonexistent_server",
            "some_tool",
            json!({})
        ...
        assert!(nonexistent_server_result.is_err(), "Nonexistent server should return error");
        ...
        let large_args_result = store.execute_tool_on_local_server(
            &session_id,
            "filesystem",
            "write_file",
            json!({
        ...
        let special_chars_result = store.execute_tool_on_local_server(
            &session_id,
            "filesystem",
            "write_file",
            json!({
        ...
        assert!(special_chars_result.is_ok(), "Special characters should work");
        ...
        assert!(read_result.is_ok(), "Reading special chars file should work");
        let read_content = read_result.unwrap();
        let content_text = read_content.get("content")
            .and_then(|c| c.as_array())
            .and_then(|arr| arr.first())
        ...
        assert!(content_text.contains("🎉"), "Special characters should be preserved");
    ...
    async fn test_multiple_sessions_concurrent_execution() {
        ...
        for (i, (_, test_dir)) in session_ids.iter().enumerate() {
            ...
            assert!(std::path::Path::new(&file_path).exists(), "Session {} file should exist", i + 1);
        }
        ...
        if let Some(content_array) = read_response.get("content").and_then(|c| c.as_array()) {
            if let Some(text_obj) = content_array.first() {
                if let Some(text) = text_obj.get("text").and_then(|t| t.as_str()) {
                    assert_eq!(text, test_content, "File content should match what was written");
    ...
    async fn test_performance_and_error_handling() {
        ...
        println!("✅ Sequential operations completed in {:?}", sequential_duration);
        ...
        println!("📊 Performance consistency: {:.2}x ratio between sequential and rapid operations", ratio);
        
        // Error Handling Test 1: Invalid file paths
        println!("❌ Test 3: Error handling - invalid file paths");
        let invalid_path_result = store.execute_tool_on_local_server(
            &session_id,
            "filesystem",
            "write_file",
            json!({
        ...
        let large_file_result = store.execute_tool_on_local_server(
            &session_id,
            "filesystem",
            "write_file",
            json!({
        ...
        assert!(large_file_result.is_ok(), "Large file operations should work");
        ...
        let missing_file_result = store.execute_tool_on_local_server(
            &session_id,
            "filesystem",
            "read_file",
            json!({
        ...
        for _i in 0..20 {
            let result = store.execute_tool_on_local_server(
                &session_id,
                "filesystem",
                "list_directory",
                json!({
        ...
        println!("✅ Stress test completed: {}/20 requests succeeded in {:?}", 
                success_count, stress_duration);
        assert!(success_count >= 18, "At least 90% of stress test requests should succeed");
