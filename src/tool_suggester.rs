...
impl ToolSuggester {
    ...
    fn init_keyword_mappings(&mut self) {
        ...
        self.tool_keywords.insert(
            ...
            vec!["read file", "load file", "open file", "file contents", "read from"].iter()
                .map(|s| s.to_string()).collect()
        );

        self.tool_keywords.insert(
            ("filesystem".to_string(), "write_file".to_string()),
            vec!["write file", "save file", "create file", "write to", "save to", "generate file"].iter()
                .map(|s| s.to_string()).collect()
        );

        self.tool_keywords.insert(
            ("filesystem".to_string(), "list_directory".to_string()),
            vec!["list files", "directory contents", "folder structure", "ls", "dir"].iter()
                .map(|s| s.to_string()).collect()
        );
        ...
        self.tool_keywords.insert(
            ...
            vec!["git status", "check changes", "uncommitted", "modified files"].iter()
                .map(|s| s.to_string()).collect()
        );

        self.tool_keywords.insert(
            ("git".to_string(), "git_commit".to_string()),
            vec!["commit", "git commit", "save changes", "checkpoint"].iter()
                .map(|s| s.to_string()).collect()
        );
        ...
        self.tool_keywords.insert(
            ...
            vec!["remember", "store information", "save to memory", "create entity", "knowledge graph"].iter()
                .map(|s| s.to_string()).collect()
        );

        self.tool_keywords.insert(
            ("memory".to_string(), "read_graph".to_string()),
            vec!["recall", "retrieve memory", "what do you know", "read memory", "get information"].iter()
                .map(|s| s.to_string()).collect()
        );
        ...
        self.tool_keywords.insert(
            ...
            vec!["create issue", "github issue", "bug report", "feature request", "track issue"].iter()
                .map(|s| s.to_string()).collect()
        );

        self.tool_keywords.insert(
            ("github".to_string(), "create_pull_request".to_string()),
            vec!["pull request", "PR", "merge request", "code review"].iter()
                .map(|s| s.to_string()).collect()
        );
        ...
        self.tool_keywords.insert(
            ...
            vec!["list tasks", "show tasks", "task status", "project status"].iter()
                .map(|s| s.to_string()).collect()
        );

        self.tool_keywords.insert(
            ("task-master-ai".to_string(), "add_task".to_string()),
            vec!["add task", "create task", "new task", "task for"].iter()
                .map(|s| s.to_string()).collect()
        );
        ...
        self.tool_keywords.insert(
            ...
            vec!["docker ps", "list containers", "running containers", "docker status"].iter()
                .map(|s| s.to_string()).collect()
        );

        self.tool_keywords.insert(
            ("docker".to_string(), "build_image".to_string()),
            vec!["docker build", "build image", "create image", "dockerfile"].iter()
                .map(|s| s.to_string()).collect()
        );
        ...
        self.tool_keywords.insert(
            ...
            vec!["screenshot", "capture page", "web screenshot", "browser capture"].iter()
                .map(|s| s.to_string()).collect()
        );

        self.tool_keywords.insert(
            ("puppeteer".to_string(), "navigate".to_string()),
            vec!["navigate to", "open url", "browse to", "visit page", "web scraping"].iter()
                .map(|s| s.to_string()).collect()
        );
    }
    ...
    fn init_pattern_matchers(&mut self) {
        ...
        self.patterns.push((
            ...
            vec![("redis".to_string(), "get".to_string()), ("redis".to_string(), "set".to_string())]
        ));
...
mod tests {
    ...
    fn test_filesystem_suggestions() {
        ...
        let analysis = suggester.analyze_task(
            ...
            Some("Load the configuration file and parse its contents")
        );

        assert!(!analysis.suggested_tools.is_empty());
        assert!(analysis.suggested_tools.iter().any(|s|
            s.server_name == "filesystem" && s.tool_name == "read_file"
        ));
    }
    ...
    fn test_git_suggestions() {
        ...
        assert!(analysis.suggested_tools.iter().any(|s|
            s.server_name == "git" && s.tool_name == "git_commit"
        ));
    }
