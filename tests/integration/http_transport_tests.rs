...
async fn test_rustdocs_sse_transport() {
                ...
                if content_type.contains("text/event-stream") {
                    ...
                    if let Some(data_line) = body.lines().find(|line| line.starts_with("data: ")) {
                        ...
                        if let Some(session_param) = endpoint_path.split("sessionId=").nth(1) {
                            ...
                            let message_result = client.post(&message_url)
                                .json(&tools_request)
                                .send()
                                .await;
    ...
    match result.await {
        ...
        Ok(Err(e)) => println!("SSE transport test failed (expected in some environments): {}", e),
...
async fn test_solana_direct_http() {
    ...
    match result.await {
        Ok(Ok(response)) => {
            assert!(response.status().is_success() || response.status().is_client_error());
            println!("Solana direct HTTP test passed: status {}", response.status());
        },
        Ok(Err(e)) => {
            // Network errors are acceptable in test environment
