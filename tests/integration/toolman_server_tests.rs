...
impl ToolmanServerTest {
    ...
    pub async fn start_server(&mut self) -> Result<()> {
        println!("🚀 Starting Toolman HTTP server on port {}", self.server_port);
        ...
        let process = cmd.spawn()
            .context("Failed to spawn toolman-server")?;
    ...
    async fn wait_for_server_ready(&self) -> Result<()> {
        ...
        for attempt in 1..=max_attempts {
            ...
            match timeout(Duration::from_secs(2), self.client.get(&url).send()).await {
                ...
                Ok(Ok(response)) => {
                    println!("⏳ Attempt {}: Server responded with status {}", attempt, response.status());
                }
                Ok(Err(e)) => {
        ...
        Err(anyhow::anyhow!("Server failed to become ready after {} attempts", max_attempts))
    }
    ...
    pub async fn test_initialization(&self) -> Result<()> {
        ...
        if !response.status().is_success() {
            return Err(anyhow::anyhow!("Initialize failed with status: {}", response.status()));
        }
    ...
    pub async fn test_tools_list(&self) -> Result<Vec<String>> {
        ...
        if !response.status().is_success() {
            return Err(anyhow::anyhow!("Tools list failed with status: {}", response.status()));
        }
        ...
        println!("✅ Tools list test passed. Found {} tools: {:?}", tool_names.len(), tool_names);
        Ok(tool_names)
    }
    ...
    pub async fn test_tool_forwarding(&self) -> Result<()> {
        ...
        if !response.status().is_success() {
            return Err(anyhow::anyhow!("Tool call failed with status: {}", response.status()));
        }
        ...
        if let Some(result) = tool_response.get("result") {
            ...
            return Err(anyhow::anyhow!("Tool response missing both result and error"));
        }
    ...
    pub async fn test_invalid_tool(&self) -> Result<()> {
        ...
        if !response.status().is_success() {
            return Err(anyhow::anyhow!("Invalid tool request failed with status: {}", response.status()));
        }
        ...
        let result = error_response["result"].as_object()
