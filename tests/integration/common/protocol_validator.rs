...
impl ProtocolValidator {
    pub async fn validate_initialization(server: &mut TestServer) -> Result<Value> {
        println!("Validating initialization for server: {}", server.config.name);
        ...
        if let Some(protocol_version) = response.get("protocolVersion") {
            ...
            return Err(anyhow::anyhow!("Missing protocolVersion in initialize response"));
        }

        if let Some(capabilities) = response.get("capabilities") {
            println!("Server capabilities: {}", capabilities);
        } else {
            return Err(anyhow::anyhow!("Missing capabilities in initialize response"));
        }
    ...
    pub async fn validate_tool_listing(server: &mut TestServer) -> Result<Vec<String>> {
        ...
        let response = server.list_tools().await
    ...
    pub async fn validate_basic_tool_call(server: &mut TestServer, tool_name: &str, args: Value) -> Result<Value> {
        ...
        println!("Tool call successful: {}", serde_json::to_string_pretty(&response)?);
        Ok(response)
    }

    pub async fn validate_error_handling(server: &mut TestServer) -> Result<()> {
        println!("Validating error handling for server: {}", server.config.name);
        ...
        // Test invalid method
        let result = server.client.send_request("invalid_method", json!({})).await;
        if result.is_ok() {
    ...
    pub async fn run_comprehensive_validation(server: &mut TestServer) -> Result<ValidationReport> {
        ...
        match Self::validate_initialization(server).await {
            Ok(response) => {
                ...
                report.protocol_version = response.get("protocolVersion")
                    .and_then(|v| v.as_str())
                    .unwrap_or("unknown")
                    .to_string();
            }
...
impl ValidationReport {
    ...
    pub fn print_summary(&self) {
        ...
        println!("Initialization: {}", if self.initialization_passed { "✅ PASS" } else { "❌ FAIL" });
        println!("Tools List: {}", if self.tools_list_passed { "✅ PASS" } else { "❌ FAIL" });
        println!("Error Handling: {}", if self.error_handling_passed { "✅ PASS" } else { "❌ FAIL" });
        ...
        println!("Overall Status: {}", if self.is_healthy() { "✅ HEALTHY" } else { "❌ UNHEALTHY" });
        println!("====================================\n");
    }
}
